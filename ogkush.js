...
const localStoragePrefix = UNIVERSE + "-" + PLAYER_ID + "-";/// Page Context Imports
...
class OGInfinity {
  ...
  async updateServerSettings(force = false) {
    ...
    let settingsUrl = `/api/s${universeNum}/${lang}/serverData.xml`;
  ...
  fixRedirectGalaxy() {
    history.pushState({}, null, `?page=ingame&component=galaxy&galaxy=${galaxy}&system=${system}`);
  }
  ...
  fetchAndConvertRC(messageId) {
    const url = `?page=messages&messageId=${messageId}&tabid=21&ajax=1`;
    return fetch(url)
      .then((rep) => rep.text())
      .then((str) => {
  ...
  expedition() {
    if (this.page == "fleetdispatch" && fleetDispatcher.shipsOnPlanet.length !== 0 && !fleetDispatcher.isOnVacation) {
      ...
      btnExpe.addEventListener("click", async () => {
        ...
        this.onFleetSentRedirectUrl =  link;
        this.expedition = false;
      });
  ...
  async getLifeformBonus() {
    ...
    return fetch(
      `?page=ingame&component=lfbonuses`,
  ...
  spyTable() {
    if (this.page == "fleetdispatch" && this.mode == ogiMode.RAID) {
      let link =  "?page=ingame&component=messages";
  ...
  getLocalStorageSize() {
    var other = 0;
    var ogi = 0;for(var x in localStorage){if(!x.startsWith(localStoragePrefix)){continue;}
  ...
  purgeLocalStorage(){for(var x in localStorage){if(!x.startsWith(localStoragePrefix)){continue;}if(x!=localStoragePrefix+"ogk-data"){
    ...
    if (this.incomingHostileFleetPerPlanets) {
      ...
      const createAlertIcon = (type, planetOrMoonId, fleetCount) => {
        ...
        const alert = DOM.createDOM("a", {
          href: `?page=ingame&component=fleetdispatch&cp=${planetOrMoonId}`,
  ...
  getAllianceClass() {
    fetch("?page=ingame&component=resourcesettings")
      .then((rep) => rep.text())
      .then((str) => {
    ...
    document.querySelectorAll(".planet-koords").forEach((planet) => {
      ...
      const createFleetIcon = (standardUnitSum, planetOrMoonId, iconClass, redirect) => {
        const fleetIcon = DOM.createDOM("a", {
          ...
          href: `?page=ingame&component=${redirect ? "fleetdispatch" : this.page}&cp=${planetOrMoonId}`,
        });
    ...
    const updateProgressIndicators = () => {
      ...
      const createConstructionIcon = (elem, planetOrMoonId, techName, iconClass, component, addToolTip, redirect) => {
        const constructionIcon = DOM.createDOM("a", {
          ...
          href: `?page=ingame&component=${redirect ? component : this.page}&cp=${planetOrMoonId}`,
        });
