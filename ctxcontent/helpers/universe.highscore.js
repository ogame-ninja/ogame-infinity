 ...
 */
export function getPlayersHighscore(protocol, host, universe, universeNum, universeLang) {
  const playerTypePromises = requestHighscore(protocol, host, universe, HIGHSCORE_CATEGORY.PLAYER, universeNum, universeLang);
...
export function getAllianceHighscore(universe) {
  const allianceTypePromises = requestHighscore(protocol, host, universe, HIGHSCORE_CATEGORY.ALLIANCE, universeNum, universeLang);
 ...
 */
function requestHighscore(protocol, host, universe, category, universeNum, universeLang) {
  ...
  return typesToUpdate.flatMap((type) => {
    return requestOGameHighScore(protocol, host, universe, category, type, universeNum, universeLang).then(toHighscoreTypeMap);
  });
}
